...
class Wheel {
    ...
    endDisplay() {
        return '|raw|<div style="padding: 5px; background-color: black; color: red; border: solid, 1px, red; font-size: 15px;">The wheel has landed on: <em><strong>' + this.wheelValue + '</strong></em><br>This means you have either lost or won: ' + this.wheelValue; + ' Shekels.</div>'
    }
}
...
exports.commands = {
    wheel: {
        ...
        join: function(target, room, user) {
            ...
            Db('money').set(room.wheelHost, Db('money').get(room.wheelHost) + payout * -1);
            delete room.wheel;
        },
        ...
        list: function(target, room, user) {
            if (!this.runBroadcast()) return;
            this.sendReplyBox('<c>Wheel of Misfortune</c><br />' +
